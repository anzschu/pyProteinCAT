...
def angles(session, proteinfile):
...
def generateVectorFile(proteinfile):
    ...
    hx, hy, hz = structure.hydrophobicvector()/200
...
def main(fname):
    drawDipole(session, fname)
    drawHydrophobe(session, fname)

parser = argparse.ArgumentParser()
parser.add_argument("fname", help="pass filename to script")
